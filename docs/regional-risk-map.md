...
- The dashboard calls GEE directly in `live` mode when credentials are configured, using a bounded region and the latest common available date across the configured NDVI, rainfall and LST collections. If GEE is unavailable, it falls back to local cache and then demo data.
