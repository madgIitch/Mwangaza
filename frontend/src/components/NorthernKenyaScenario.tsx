...
const units = [
  { id: "KEN-023", name: "Turkana", severity: "critical", score: 0.84, ndvi: -0.42, rainfall: -0.48, report: "report-KEN-023-demo" },
  { id: "KEN-010", name: "Marsabit", severity: "warning", score: 0.68, ndvi: -0.31, rainfall: -0.35, report: "report-KEN-010-demo" },
  { id: "KEN-011", name: "Isiolo", severity: "watch", score: 0.51, ndvi: -0.2, rainfall: -0.24, report: "report-KEN-011-demo" }
] as const;
  ...
  return (
    <section className="kenya-scenario" aria-label="Northern Kenya demo scenario">
      ...
      <div className="kenya-scenario-grid">
        ...
        <div aria-live="polite"><h3>{selected.name} · {selected.id}</h3><p><strong>Composite score:</strong> {selected.score}</p><p>NDVI anomaly {selected.ndvi} · Rainfall anomaly {selected.rainfall}</p><p><strong>Report:</strong> {selected.report} · {selected.id}</p></div>
